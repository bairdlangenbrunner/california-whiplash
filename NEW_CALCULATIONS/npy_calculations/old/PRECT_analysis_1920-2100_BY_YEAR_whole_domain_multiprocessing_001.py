...
################################################################################
################################################################################
file_dir = '/Users/baird/google_drive/_data_original/NCAR_LENS/daily/PRECT/hist_and_rcp85_CA_REGION/'
...
save_dir = '/Users/baird/google_drive/_data_original/NCAR_LENS/daily/PRECT/calculated_npy_files/whole_domain/'
...
ncfile = netCDF4.Dataset(file_dir + file_list[0])
...
mgrid = numpy.meshgrid(range(PRECT_lat.size), range(PRECT_lon.size), indexing='ij')

#print(PRECT_lat.size, PRECT_lon.size)
#exit()

# create entire list of file names
"""
all_files_list = []
for i in range(PRECT_lat.size):
	for j in range(PRECT_lon.size):
		ensemble_member = file_list[0].split('.')[1]
		lat_idx, lon_idx = i,j
		all_files_list.append('member_'+ensemble_member+'_latidx_'+'{:02d}'.format(lat_idx) + '_lonidx_'+'{:02d}'.format(lon_idx)+'_years_' + str(year_start)+'-'+str(year_end)+'_threshold_' + str(threshold)+'mmday_'+region+'.npy')
files_exist = numpy.array([os.path.isfile(save_dir + file) for file in all_files_list])
"""
def main():
	pool = multiprocessing.Pool(processes=1)
	input = zip(mgrid[0].flatten(), mgrid[1].flatten())
	pool.map(process_files, input)
