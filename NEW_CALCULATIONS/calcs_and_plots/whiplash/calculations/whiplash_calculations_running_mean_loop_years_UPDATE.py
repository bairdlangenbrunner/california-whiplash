...
year_start_list = numpy.array((1920,1950,1980,2010,2040,2070,2002))
year_end_list = numpy.array((1950,1980,2010,2040,2070,2100,2032))
...
for latlon_idx in range(len(latlon_indices)):
	...
	for yr_idx in range(year_start_list.size):
		...
		#print(whiplash_ratio)
		whiplash_ratio = rcp_freq/pic_freq
		whiplash_ratios_all[yr_idx, latlon_idx] = whiplash_ratio
		#print(whiplash_ratios_all.shape)
		#whiplash_ratios_all[0:5] = whiplash_ratio
	#print(whiplash_ratios_all)

print('Now saving all files!')
for yr_idx in range(year_start_list.size):
	whiplash_label = str(year_start_list[yr_idx])+'-'+str(year_end_list[yr_idx])
	whiplash_ratios_yearly = whiplash_ratios_all[yr_idx,:]
	whiplash_ratios_all_df = pandas.DataFrame(whiplash_ratios_yearly, columns=[whiplash_label])
	whiplash_ratios_all_df.to_csv('csv_files_UPDATE/store_all_whiplash_ratios_dataframe_' + str(hi_perc)+str(lo_perc) + '_'+region+'_'+str(year_start_list[yr_idx]) + '-'+str(year_end_list[yr_idx])+'_windowsize_'+str(window)+'.csv')