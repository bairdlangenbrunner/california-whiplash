...
for yr_idx in [0]:#range(6):
	...
	for latlon_idx in range(len(latlon_indices)):
		...
		# now calculate window running mean for each ensemble member
		...
		for i in range(len(ensemble_names)):
			...
			while s<len(season_strings[year_start_index:year_end_index])-1:
				...
				#	whiplash_count+=1
				#	s+=1
				if ((seasonal_total_separate_ensembles[i][s]>pic_hi)&(seasonal_total_separate_ensembles[i][s+1]<pic_lo)):
					whiplash_count+=1
					s+=1
				else:
					s+=1
	...
	whiplash_ratios_all_df.to_csv('store_all_whiplash_ratios_dataframe_'+str(hi_perc)+str(lo_perc)+'_'+region+'_'+str(year_start_list[yr_idx])+'-'+str(year_end_list[yr_idx])+'_wet2dry.csv')